...
This repository describes the OHDSI HADES results model. These are the
tables that are populated by the various HADES packages, often as part
of a [Strategus](https://ohdsi.github.io/Strategus/) execution. The
repository also contains an R package with model resources and
utilities.
...
This repository follows the standard R package structure. Model
resources live under `inst/`:

- `inst/modules/`: versioned module definitions
  (`<ModuleName>/v<semver>/definition.yaml`)
...
- `current_csvs/`: legacy CSV source material used for
...
See
[MAINTAINER.md](https://ohdsi.github.io/HadesResultsModel/MAINTAINER.md)
for release manifests, DDL generation, and module versioning.
...
Module upgrades may include migration SQL for moving from an older
version to a newer one.

- Migration scripts are stored alongside the target module version, for
  example `inst/modules/CohortGenerator/v1.0.0/migration.sql`.
- Migration SQL is OHDSI SQL and can be translated with SqlRender before
  execution.
- Major version upgrades should remove deprecated fields and may add new
  tables or columns through migration scripts.
